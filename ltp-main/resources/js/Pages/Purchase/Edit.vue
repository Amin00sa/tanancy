...
export default{
    ...
    setup(props){
        const form = useForm({
            marche_id:props.purchase.marche.id,
            supplier_id:props.purchase.supplier.id,
            bank_id:props.purchase.bank.id,
            products:props.purchase.products,
            taux_tva:props.purchase.payment.taux_tva,
            date:props.purchase.date ? moment(props.purchase.date).format('YYYY-MM-DD') : null,
            paiement_immediat:props.purchase.payment.paiement_immediat ? true : false,
            echeance_date:props.purchase.payment.echeance_date ? moment(props.purchase.payment.echeance_date).format('YYYY-MM-DD') : null,
            total:null,
            type:null,
            motif:props.purchase.payment.motif_virement,
        })

        return{form}
    },
    mounted(){
        if(this.purchase.payment.type == 'cheque'){
            this.form.type = 'Chèque'
        }
        if(this.purchase.payment.type == 'espece'){
            this.form.type = 'Espèce'
        }
        if(this.purchase.payment.type == 'virement'){
            this.form.type = 'Virement'
        }
        if(this.purchase.payment.type == 'versement'){
            this.form.type = 'Versement'
        }
        if(this.purchase.payment.type == 'effet'){
            this.form.type = 'Effet'
        }
    },
    computed:{
        totalht(){
    ...
    methods:{
        submit(){
            ...
            this.$refs.formulaire.validate().then(() => {
                if(this.valid){
                    this.loading = true
                    this.$inertia.post(`/purchases/${this.purchase.id}`, {
                        _method: "put",
                        ...
                        paiement_immediat: this.form.paiement_immediat,
                        echeance_date: this.form.echeance_date,
                        date: this.form.date,
                        proof_file: this.files[0],
                        total:this.totalht,
...
<template>
    <AppLayout title="Modification d'achat">
        <template #header>
            <h2 class="text-3xl text-gray-800 leading-tight"><span class="font-semibold">Modification d'achat :</span> {{purchase.numero}}</h2>
        </template>
        <v-form :disabled="loading"  v-model="valid" @submit.prevent="submit" ref="formulaire">
            ...
            <v-sheet color="grey-white" class="p-6 mt-4" border rounded>
                ...
                >
                    ...
                    md="6"
                    >
                        <v-file-input label="Justificatif" v-model="files" accept="image/*,application/pdf"></v-file-input>
                    </v-col>
                </v-row>
            </v-sheet>

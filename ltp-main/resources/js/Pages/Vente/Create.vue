    ...
    methods:{
        submit(){
            ...
            this.$refs.formulaire.validate().then(() => {
                if(this.valid){
                    this.form.post(route('ventes.store'));
                }
            })  
        },
        async save(){
...
<template>
    <AppLayout title="Création d'une nouvelle vente">
        <template #header>
            <h2 class="font-semibold text-3xl text-gray-800 leading-tight">Ajout d'une nouvelle vente</h2>
        </template>
